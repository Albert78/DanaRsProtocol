...
/**
 * DanaRS/Dana-i packet encryption and checksum implementation.
 *
 * This class owns session state such as pairing keys, time/password-derived default encryption, and
 * second-level RSv3/BLE5 encryption.
 */
@OptIn(ExperimentalUnsignedTypes::class)
class DanaRsBleEncryption {
    ...
    var securityVersion: DanaRsEncryptionType = DanaRsEncryptionType.ENCRYPTION_DEFAULT
    ...
    fun getEncryptedPacket(opcode: Int, bytes: ByteArray?, deviceName: String?): ByteArray {
        ...
        when (opcode) {
            ...
            DANAR_PACKET__OPCODE_ENCRYPTION__TIME_INFORMATION    -> {
                ...
                if (securityVersion == DanaRsEncryptionType.ENCRYPTION_BLE5) {
            ...
            DANAR_PACKET__OPCODE_ENCRYPTION__GET_EASY_MENU_CHECK -> {
                ...
                encodeArrayBySn(array)
                if (securityVersion == DanaRsEncryptionType.ENCRYPTION_DEFAULT) {
    ...
    fun getDecryptedPacket(bytes: ByteArray): ByteArray? {
        ...
        encodeArrayBySn(array)
        if (connectionState == 2 && securityVersion == DanaRsEncryptionType.ENCRYPTION_DEFAULT) {
            encodeArrayByTime(array, timeInfo)
            encodeArrayByPassword(array, password)
            encodeArrayByCfPassKey(array)
        }
        ...
        if (arr[0] == DANAR_PACKET__TYPE_ENCRYPTION_RESPONSE &&
            ...
            when (securityVersion) {
                DanaRsEncryptionType.ENCRYPTION_RSv3 -> connectionState = if (pairingKey == null && randomPairingKey == null) 1 else 2
                DanaRsEncryptionType.ENCRYPTION_BLE5 -> connectionState = if (ble5PairingKey == null) 1 else 2
    ...
    fun setEnhancedEncryption(securityVersion: DanaRsEncryptionType) {
    ...
    fun encryptSecondLevelPacket(bytes: ByteArray): ByteArray {
        ...
        if (securityVersion == DanaRsEncryptionType.ENCRYPTION_RSv3) {
        ...
        } else if (securityVersion == DanaRsEncryptionType.ENCRYPTION_BLE5) {
            if (array[DANAR_PACKET__START_POS] == DANAR_PACKET__START_PACKET && array[DANAR_PACKET__START_POS + 1] == DANAR_PACKET__START_PACKET) {
    ...
    fun generateCrc(buffer: UByteArray): UInt {
        ...
        for (i in 0..<buffer.size) {
            ...
            when (securityVersion) {
                DanaRsEncryptionType.ENCRYPTION_DEFAULT ->
                ...
                DanaRsEncryptionType.ENCRYPTION_BLE5    ->
